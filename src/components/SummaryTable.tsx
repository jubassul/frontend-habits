...
const amountOfDaysToFill = minimumSumaryDatesSize - summaryDates.length; //126-16 (110 só para aparecer)
    ...
    <div className="w-full flex">
      <div className="grid grid-rows-7 grid-flow-row gap-3">
        {weekDays.map((weekDay, i) => {
          return (
            <div
              key={`${weekDay}-${i}`}
              className="text-zinc-400 font-bold text-xl h-10 w-10 flex items-center justify-cente"
            >
              {weekDay}
            </div>
          );
      ...
      <div className="grid grid-rows-7 grid-flow-col gap-3">
        {summaryDates.map((date) => {
         ...
         return <HabitDay date={date}  completed={dayInSummary?.completed} amount={dayInSummary?.amount} />;
        })}
